...
/**
 * @brief       定时器TIMX 通道Y 输出指定个数PWM 初始化函数
 * @note
 *              定时器的时钟来自APB1,当D2PPRE1≥2分频的时候
 *              定时器的时钟为APB1时钟的2倍, 而APB1为120M, 所以定时器时钟 = 240Mhz
...
void gtim_timx_npwm_chy_init(uint16_t arr,uint16_t psc)
{
	...
	g_timx_npwm_chy_handle.Init.ClockDivision = TIM_CLOCKDIVISION_DIV1;		/* 分频 */
	...
	g_timx_oc_npwm_chy_handle.Pulse = arr/2;  														/* 设置比较值,此值用来确定占空比，默认比较值为自动重装载值的一半,即占空比为50% */   
	...
	HAL_TIM_PWM_Start(&g_timx_npwm_chy_handle, GTIM_TIMX_NPWM_CHY);				/*开启通道y*/
...
void HAL_TIM_PWM_MspInit(TIM_HandleTypeDef *htim)
{
    if (htim->Instance == GTIM_TIMX_NPWM)
    {
        ...
        HAL_GPIO_Init(GTIM_TIMX_NPWM_CHY_GPIO_PORT, &gpio_init_struct);		/* 初始化GPIO */
    }
}
...
{
    if (htim == (&g_timx_npwm_chy_handle)) 		/* 输出指定个数PWM实验回调执行的内容 */
    {
        uint16_t npwm = 0;
