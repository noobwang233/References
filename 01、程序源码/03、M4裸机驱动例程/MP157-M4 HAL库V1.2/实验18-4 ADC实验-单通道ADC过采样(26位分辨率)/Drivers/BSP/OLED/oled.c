...
static void oled_data_out(uint8_t data)
{
	...
	GPIOH->ODR &= ~(0XF << 9);       	/* 清空PH的第12~9位 */
    GPIOH->ODR |= dat << 9;         /* 将D[3:0]分配给PH[12:9] */
	
	GPIOH->ODR &= ~(0X1 << 14);     	/* 清空PH的第14位 */
    GPIOH->ODR |= ((data >> 4) & 0x01) << 14;	/* 将D4给PH14 */
	
	GPIOI->ODR &= ~(0X1 << 4);      						/* 清空PI4 */
    GPIOI->ODR |= ((data >> 5) & 0x01) << 4;	/* 将D5给PI4 */
	
	GPIOB->ODR &= ~(0X1 << 8);       						/* 清空PB8 */
    GPIOB->ODR |= ((data >> 6) & 0x01) << 8;

	GPIOE->ODR &= ~(0X1 << 6);      						/* 清空PE6 */
    GPIOE->ODR |= ((data >> 7) & 0x01) << 6;	/* 将D7给PE6 */
#if 0
...
{
    oled_data_out(data);/* 向OLED输出一个8位字节 */
    OLED_RS(cmd);				/* cmd为0,表示命令;cmd为1,表示数据 */
    OLED_CS(0);					/* 片选引脚拉低，选中SSD1306 */
    OLED_WR(0);					/* WR先拉低 */
    OLED_WR(1);					/* WR再拉高，变成上升沿了，使字节写入到数据线D[7：0]上 */
    OLED_CS(1);					/* 片选引脚拉高，关闭片选 */
    OLED_RS(1);					/* 即DC设置为1,DC电平恢复至初始态 */
}
#else   /* 使用SPI驱动OLED */
/**
 * @brief       向OLED写入一个字节
...
static void oled_wr_byte(uint8_t data, uint8_t cmd)
{
    ...
    for (i = 0; i < 8; i++)/* SPI是串行，一位一位地传输 */
    {
        OLED_SCLK(0);			/* SCLK为低电平，低电平开始采样数据 */
        if (data & 0x80)	/* 高位在前 */
        {
            OLED_SDIN(1);	/* 写1 */
        }
        ...
        OLED_SCLK(1);			/* SCLK为高电平 */
        data <<= 1;				/* 左移 */
    }

    OLED_CS(1);						/* 关闭片选 */
    OLED_RS(1);						/* DC电平恢复至初始态 */
}
#endif
...
/* 使能GPIOA、GPIOB、GPIOC、GPIOD、GPIOE、GPIOH和GPIOI时钟 */
	...
	HAL_GPIO_Init(GPIOH,&gpio_init_struct);   /* 初始化GPIOH */

	/* 设置PHI4 */
	gpio_init_struct.Pin=GPIO_PIN_4;
	HAL_GPIO_Init(GPIOI,&gpio_init_struct);  /* 初始化GPIOI */
	...
	OLED_SDIN(1);	/* 设置SDIN引脚 */
	OLED_SCLK(1);	/* 设置SCLK引脚 */
#endif
    OLED_CS(1);
    OLED_RS(1);
