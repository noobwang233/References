...
void adc_dma_init(uint32_t par, uint32_t mar)
{
    ...
    g_adc_dma_handle.Init.ClockPrescaler = ADC_CLOCK_SYNC_PCLK_DIV2;            /* 输入时钟2分频,即adc_ker_ck= PER/2=32MHz */
    ...
    g_adc_dma_handle.Init.Overrun = ADC_OVR_DATA_OVERWRITTEN;                   /* 有新的数据的后直接覆盖掉旧数据 */
