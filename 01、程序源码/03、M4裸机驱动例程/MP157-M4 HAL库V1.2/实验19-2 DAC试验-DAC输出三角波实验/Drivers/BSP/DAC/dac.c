 ...
 *   @note   输出频率 ≈ 1000 / (dt * samples) Khz, 不过在dt较小的时候,
 *           比如小于5us时, 由于delay_us本身就不准了(调用函数,计算等都
 *           需要时间,延时很小的时候,这些时间会影响到延时), 频率会偏小.
 * 
...
void dac_triangular_wave(uint16_t maxval, uint16_t dt, uint16_t samples, uint16_t n)
{
    ...
    { 
        DAC1->DHR12R1 = 0;  /* 先输出0 */
        for(i = 0; i < (samples / 2); i++)  /* 输出递增 */
        { 
             DAC1->DHR12R1 +=  incval;			/* 递增量incval */
             delay_us(dt);									/* 延时dt us */
        } 
        for(i = 0; i < (samples / 2); i++)  /* 输出递减 */
        {
             DAC1->DHR12R1 -=  incval;			/* 递减量incval */
             delay_us(dt);									/* 延时dt us */
        }
    }
}
