 ...
 * @param  plln: PLL3倍频系数(PLL倍频), 取值范围: 25~200（RCC_PLL3CFGR1的DIVN[8:0]位）.
 * @param  pllm: PLL3预分频系数(进PLL之前的分频), 取值范围: 1~64（RCC_PLL3CFGR1的DIVM3[5:0]位）.
 * @param  pllp: PLL3的p分频系数(PLL之后的分频), 分频后作为系统时钟, 
 *							取值范围: 1~128.(且必须是2的倍数)（RCC_PLL3CFGR2的DIVP[6:0]位）
 * @param  pllq: PLL3的q分频系数(PLL之后的分频), 取值范围: 1~128（RCC_PLL3CFGR2的DIVR[6:0]位).
 * @param	 pllfracv：PLL3的FRACV位，即乘法因子的小数部分，FRACV可介于0和213-1之间。
 *            实际上是配置RCC_PLL3FRACR的 FRACV[12:0]位。
 * @note
 *
 *              Fvco: VCO频率
...
uint8_t sys_stm32_clock_init(uint32_t plln, uint32_t pllm, uint32_t pllp, uint32_t pllq, uint32_t pllfracv)
{
	...
	rcc_osc_init_handle.PLL.PLLSource = RCC_PLL12SOURCE_HSE;/* 选择HSE为PLL1/2时钟源 */
	...
	rcc_osc_init_handle.PLL.RPDFN_DIS = RCC_RPDFN_DIS_DISABLED;/* 不使用RPDF噪声控制 */
	rcc_osc_init_handle.PLL.TPDFN_DIS = RCC_TPDFN_DIS_DISABLED;/* 不使用TPDF噪声控制 */
	 ...
	 * PLL（PLL1、PLL2、PLL3和PLL4）是否配置完成，配置完成则返回HAL_OK。
	...
	rcc_clk_init_handle.MPUInit.MPU_Clock = RCC_MPUSOURCE_PLL1;		/* MPU时钟源为PLL1P = 800MHz 			*/
	rcc_clk_init_handle.MPUInit.MPU_Div = RCC_MPU_DIV2;				/* MPUDIV 2分频 = 800 / 2 = 400MHz		*/
